                        ...
                        isSecureFlag =false;
                            ...
                            if (getSharedPreferences("myAESKeys",MODE_PRIVATE).getString(fuser.getUid() + "to" + userid,null)==null){
                                ...
                                try {
                                    ...
                                    remotePublicKey = factory.generatePublic(new X509EncodedKeySpec(Base64.getDecoder().decode(strPublicKey)));//Μετατροπή του String public Key απο string σε PublicKey
                                            Log.i("Info", "Exchange Diagnostic:   his public key generated by the String is"+remotePublicKey);

                                    //factory = KeyFactory.getInstance("DH", "BC");
                                    myPrivateKey = factory.generatePrivate(new PKCS8EncodedKeySpec(Base64.getDecoder().decode(strMyPrivateKey)));
                                    ...
                                    char[] charSharedAESKey = Hex.encodeHex(sharedAESKey.getEncoded());
                                    String strSharedAESKey = String.valueOf(charSharedAESKey);
                                ...
                                try {
                                    decodedKey = Hex.decodeHex(strSharedAESKey);
